    ...
    (async () => {
      ...
      if (targetChain === CHAIN_ID_SOLANA && originChain && originAsset) {
        ...
        try {
          ...
          // await getSolanaAssociatedTokenAcc('0xdff0cbb0a50cc3eaf8242414380eb04c7283f513', '15487864439031792820769872793764920336287736338831218889832296222679724195845', new PublicKey('9AVaowib8ePah1VdJft6mgZtYQcHgLA4y1TAEV22Jhan'))

          const targetAddressPubKey: PublicKey = await getSolanaAssociatedTokenAcc(
            '0xdff0cbb0a50cc3eaf8242414380eb04c7283f513',
            '15487864439031792820769872793764920336287736338831218889832296222679724195845',
            new PublicKey('9AVaowib8ePah1VdJft6mgZtYQcHgLA4y1TAEV22Jhan')
        )

        const targetAddressHex = uint8ArrayToHex(
          zeroPad(targetAddressPubKey.toBytes(), 32)
        )


        console.log({
          targetAddressPubKey: targetAddressHex
        })


        console.log({
          targetAddressTest: hexToUint8Array(targetAddressHex)
        })
          ...
          console.log({ asset, originAsset, tokenId, originChain })
          if (!cancelled) {
            dispatch(
              setTargetAsset(
...
const getSolanaTokenAdddress = async (originAsset: string, tokenId: string) => {
  const sourceChain = CHAIN_ID_ETHEREUM_ROPSTEN
  ...
  console.log({ solanaToken, originAssetHex, tokenId, sourceChain })
  return solanaToken
}

const getSolanaAssociatedTokenAcc = async (originAsset: string, tokenId: string, solPK: PublicKey): Promise<PublicKey> => {
